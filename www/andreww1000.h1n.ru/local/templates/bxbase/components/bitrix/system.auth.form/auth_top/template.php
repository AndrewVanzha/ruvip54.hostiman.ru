...
<form name="ff1 system_auth_form<?=$arResult["RND"]?>" method="post" target="_top" action="<?=$arResult["AUTH_URL"]?>">
<?if($arResult["BACKURL"] <> ''):?>
...
<?endforeach?>
	...
	<table width="95%">
		<tr>
			...
			<input type="text" name="USER_LOGIN" maxlength="50" value="" placeholder="<?=GetMessage("AUTH_LOGIN")?>: " size="17" />
			<script>
				BX.ready(function() {
...
<form name="ff2 system_auth_form<?=$arResult["RND"]?>" method="post" target="_top" action="<?=$arResult["AUTH_URL"]?>">
<?if($arResult["BACKURL"] <> ''):?>
	<input type="hidden" name="backurl" value="<?=$arResult["BACKURL"]?>" />
<?endif?>
	<input type="hidden" name="AUTH_FORM" value="Y" />
